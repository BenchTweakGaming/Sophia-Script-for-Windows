...
CopilotPCSupport                          = Ваш ЦП не имеет встроенного NPU, поэтому он не поддерживает никакую функцию, связанную с ИИ Windows. Нет необходимости что-либо выключать с помощью политик, поэтому только Recall и приложение Copilot будут удалены.
