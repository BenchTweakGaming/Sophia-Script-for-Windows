...
CopilotPCSupport                          = Your CPU has no a built-in NPU, so it doesn't support any Windows AI functions. No need to disable anything using GPO policies, so Recall function and Copilot application will only be removed.
